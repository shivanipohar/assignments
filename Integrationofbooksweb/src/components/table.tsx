...
//   const state = {
//     array: ["Cover", "Title", "Author", "Rating", "Price", "Description"],
//   };
...
export const StarComponent:FunctionComponent<StarProps>=({rating,minimumof,outof})=>{
 
  let finalRating=(((+rating - +minimumof)*(5-1))/(+outof - +minimumof))+1;
...
export default function Table({  handleClick }: Props) {
  ...
  //we gwt the original data of books
...
case "price":
  const[minPrice,maxPrice]=searchText.split("-");
    ...
    fetch("http://localhost:8000/books/price/"+minPrice+"/"+maxPrice)
.then(result => result.json())
.then(data => setBooks(data))
.catch(err=>console.log(err))
break;
            ...
            <Link to={url}>
              ...
              >
                <div className="book-card">
                  ...
                  {/* <p>Rating:{book.rating}</p> */}
                  <div className="star" ><span><StarComponent rating={book.rating} outof={5} minimumof={1}></StarComponent></span></div>
